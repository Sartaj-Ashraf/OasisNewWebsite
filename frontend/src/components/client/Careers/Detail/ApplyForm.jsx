    ...
    return (
      <div className="flex flex-col items-center justify-center py-10 text-center">
        ...
        <p className="text-[13px] text-slate-400 leading-relaxed max-w-[230px]">
          ...
          <span className="font-semibold" style={{ color: accentColor }}>{job.title}</span>{" "}
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input
            type="tel" placeholder="+1 234 567 890" value={form.phone}
      ...
      <div>
        ...
        >
          <input
            ref={fileRef} type="file" className="hidden" accept=".pdf,.doc,.docx"
            onChange={(e) => { setFile(e.target.files[0]); setErrors((p) => ({ ...p, file: "" })); }}
          />
          <div
            className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-colors"
            style={{
              backgroundColor: file ? `${accentColor}15` : "#f1f5f9",
              color: file ? accentColor : "#94a3b8",
            }}
          >
            <UploadIcon />
          </div>
